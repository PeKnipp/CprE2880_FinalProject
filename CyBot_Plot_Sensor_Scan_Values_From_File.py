...
def plot():
    ...
    filename = 'GUITESTING_Sensor_Data.txt' # Name of sensor data file
    ...
    for line in file_data: 
        ...
        if data == []:
            ...
            angle_degrees.append(float(data[1]))  # Column 1 holds the angle at which distance was measured
        
        elif data[0] == "Distance:":
            distance.append(float(data[1]))  # Column 1 holds the distance that was measured at a given angle      

        elif data[0] == "Object_Angle:":
            object_angles.append(float(data[1]))

        elif data[0] == "Object_Distance:":
            object_distances.append(float(data[1]))
            

    distance = np.array(distance) / 100.0 #Convert distance to a numpy array
    object_distances = np.array(object_distances) /100.0
    ...
    object_angles = np.array(object_angles)
    object_radians = (np.pi/180) * (object_angles)

    # Create a polar plot
    fig, ax = plt.subplots(subplot_kw={'projection': 'polar'}) # One subplot of type polar
    ax.plot(angle_radians, distance, color='b', linewidth=2.0)  # Plot distance verse angle (in radians), using blue, line width 4.0
    ax.scatter(object_radians, object_distances, color='y', linewidth=4.0) #Highlight the objects using red
    ...
    ax.set_rticks([.5, 1, 1.5, 2, 2.5])   # Set plot "distance" tick marks at .5, 1, 1.5, 2, and 2.5 meters
